...
export async function DELETE(req) {
  ...
  console.log("Table to drop:", primaryValue);
  ...
  try {
    if (tableName === 'ref_test_type') {
      const deleteJunct = `DELETE FROM junction_table WHERE test_name = ?`;
      await db.query(deleteJunct, [primaryValue]);

      const deleteRef = `DELETE FROM ?? WHERE ?? = ?`;
      await db.query(deleteRef, [tableName, primaryKey, primaryValue]);


      const deleteTable = `DROP TABLE ??`;
      await db.query(deleteTable, [primaryValue]);
    } else {
