...
const TestForm = ({ onClose }) => {
    ...
    const restrictedTableNames = [
        "user", "admin", "SELECT", "DROP", "INSERT", "UPDATE", "DELETE", "junction_table", 
        "appointment", "bill", "person", "patient", "ref_test_type"
    ];
    ...
    const validateTableName = () => {
        ...
        if (restrictedTableNames.includes(tableName.toLowerCase())) {
            setError(`Table name "${tableName}" is not allowed.`);
            return false;
        }
        ...
        if (!validNameRegex.test(tableName)) {
            setError('Table name must start with a letter or underscore and can only contain letters, numbers, and underscores.');
            return false;
        }
            ...
            <div className="test-form">
                <form className="pop-up-form" onSubmit={handleSubmit}>
                    ...
                    <div className="form-group">
                        <h3 className="text-medium-white-bold">Test Name:</h3>
                        <input
                            type="text"
                            value={tableName}
